...
Two primary frame extraction techniques were explored, uniform sampling and optical flow method. The former technique, evenly splits a video into its corresponding frames by using a pre-calculated step size determined by the number of frames the video has. The second technique leveraged optical flow, a method that assigns weights to frames determined by the motion of pixels between them. With this approach, frames with the greatest weightings were chosen for extraction, resulting in capturing only action-specific frames.


	\begin{figure}[h!]
		\centering
		\includegraphics[width=\linewidth]{content/chapters/4_Methodology/figures/kickflip_sequence.jpg}
		\caption{Breakdown of kickflip into individual frames.}
		\label{fig:seq1}
	\end{figure}
	
	\begin{figure}[h!]
		\centering
		\includegraphics[width=\linewidth]{content/chapters/4_Methodology/figures/kickflip_sequence_opticalFlow.jpg}
		\caption{Optical flow visualisation of a kickflip.}
		\label{fig:seq2}
	\end{figure}
...
To thoroughly evaluate the proposed Skateboard trick recognition models in this study, various evaluation metrics were employed. These metrics provide valuable insights on the ability for these models to correctly classify skateboard tricks. 
