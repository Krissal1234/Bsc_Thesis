...
the scope of this study, only a subset of this data was included in the final
dataset. Ultimately, the final dataset consisted of 128 videos per class,
totalling 384 videos.

\subsection{Labelling techniques}
This study explores two primary labelling techniques: folder-based and
...
`Kickflip', as showcased in Figure~\ref{fig:SingleVsMultiFrameKickflip}. By
...
another trick due to its subjective nature. Whereas, providing the model with
...
together characterises the full skateboard trick.
...
\begin{figure}[h]
  ...
  \end{subfigure}
  \caption{Comparison of a single frame and multiple sequential frames. Reproduced from~\cite{Titus}.}\label{fig:SingleVsMultiFrameKickflip}
\end{figure}
...
this technique being applied to the `kickflip' trick.
...
using Equation~\ref{eq:normalisation}. Normalising data in this manner ensured
...
\section{Feature Extraction with Transfer Learning}
...
that was then fine-tuned to the domain of skateboard trick recognition.
...
of 16 convolutional and fully connected (FC) layers with repeated use of
3$\times$3 convolutional filters to preserve spatial information. Furthermore,
VGG16 has proved to be effective for feature extraction in previous literature,
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth}{>{\RaggedRight}Xcc}
		...
		\bottomrule
	\end{tabularx}
	\captionof{table}{Characteristics of Transfer Learning models VGG16 and ResNet50.}\label{tab:TL-models}
\end{table}
...
\section{Dimensionality Reduction}\label{4_dimensionality_reduction}
...
extracts features from the preprocessed frames, before feeding them into their
respective sequence models.

The accuracy of these models is then evaluated on a held-out test set, to
...
F1-score. These metrics provided insightful information into the
model's ability to correctly categorise various tricks.

The metrics are defined as follows, described in
the context of the `kickflip's class and tabulated in a confusion matrix.
