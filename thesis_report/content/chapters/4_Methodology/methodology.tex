...
extraction, resulting in capturing frames with the most movement. 
...
In the context of video action recognition, the final four fully-connected (FC) layers of these models which are typically used for classification were removed. This was crucial in order to adapt these models for feature extraction, as the aim was not to classify each frame in the video, but to extract a rich collection of features. These features in the form of vectors would not only encapsulate the visual information present in each frame, but also more abstract ideas such as low-level information (like edges/shapes) to higher level features like objects and even actions themselves. By removing the final classification layers, both models resulted in a feature vector shape of 7x7x512.



\subsection{Sequence Modelling}
%Talk about LSTM and BiLSTM
\subsection{Overall Architectures}
%Talk about the general models:
%VGG-LSTM
%Resnet50-BiLSTM
%VGG-BiLSTM
%Resnet50-LSTM
