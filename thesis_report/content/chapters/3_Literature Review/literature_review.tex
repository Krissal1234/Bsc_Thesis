...
revolutionalised sectors such as healthcare and security as demonstrated in the
...
The research conducted by K. Host and M. Ivašić-Kos (2022) in
\cite{HARinSportsComputerVision} along with Wu et al. (2022) in
...
AR models in the field of sports.
...
The domain of AR comes with many challenges as described by
Zhang et al. (2017)
...
systems, most situations that benefit from AR adopt
...
their tricky dynamic backgrounds present in the video footage. Zhang et al. also
...
The camera distance, results in smaller subjects, making a detailed analysis of
human movements more challenging while lower-quality videos further complicate
the task for Human Activity Recognition (HAR) systems % by ... continue this, by distorting the images.
...
likelihood of overfitting as highlighted in prior works,
\cite{DataAugmentationCanImproveRobustness,AnOverviewOfOverfittingAndItsSolutions}.
In the realm of Skateboard trick classifiers, Shapiee et al (2020)
\cite{skatePaper1} effectively employed data augmentation techniques to expand
their dataset, demonstrating the application of these methods in improving model
performance for trick classification.
...
Xudong Jiang (2009)
\cite{FeatureExtractionForImageRecognitionAndComputerVision}, describes feature
...
for classification. The study by Manjunath Jogin et al. (2018)
\cite{FeatureExtractionUsingCNNandDeepLearning} highlights the effectiveness of
...
transfer learning as detailed by the studies by Weiss et al. (2016)
\cite{ASurveyOfTransferLearning} and Soni et al. (2018)
\cite{ApplicationAndAnalysisOfTransferLearningSurvey} involves using a
...
The study by Sargano et al. (2017)
...
Their approach showcases the resource-efficient and time-saving nature of
transfer learning, evident in their impressive accuracies of 98.15\% and
91.47\%.

Moreover, research on transfer learning is not unique within the field of
skateboard trick classification. Two other studies by Shapiee et al. (2020)
\cite{skatePaper1} and Hanciao Chen (2023)\cite{SkateboardAIPaper} have employe
this approach and achieved high accuracies. However, a more in-depth analysis of
these papers is detailed in section \ref{advancements_in_trickclassification}.
These studies strongly suggest exploring the use of various pre-trained models
for feature extraction in the development of a skateboard trick classifier.
...
rapid movements, potential background noise and camera angles. This section explores
...
\begin{description}
	...
	tasks.
	\item[2. Bi-directional Long-Short-Term Memory (BiLSTM) network:] The authors
...
outlined by Abdullah et al. (2021)
...
Hanciao Chen (2023) \cite{SkateboardAIPaper}.
...
MATLAB 2015 and Signal Processing Toolbox. These signals were then
...
\begin{figure}[htbp]
	...
	\caption{add caption and add reference in paragraph}
	\label{fig:signals}
  \end{figure}

\subsection{Computer Vision-based Approaches}
The study by Shapiee et al. (2020) \cite{skatePaper1} leverages a custom data
set comprising videos capturing the execution of five distinct skateboard
tricks, each attempted five times. Each video spans two to three seconds,
yielding a total of 750 images by extracting 30 frames per video. This study
made use of data augmentation techniques to expand their dataset further.
Consequently, they introduced an additional 2,250 images, achieving 3,000 images
in their data set. Shapiee et al. utilised Transfer Learning (TL) combined with
$k$-Nearest Neighbour ($k$-NN) fusion pipelines. They opted for a pre-trained
models like MobileNet, NasNetMobile and NasNetLarge for feature extraction
followed by a $k$-NN classification algorithm to identify the tricks. The
researchers. As a result, this technique demonstrated high classification
accuracies, with MobileNet achieving 95\%, NASNetMobile 92\% and NASNetLarge
90\%.

On the other hand, Chen (2023) \cite{SkateboardAIPaper}
compiled a comprehensive data set by collecting videos from multiple platforms,
including YouTube, Twitter and Instagram. Furthermore, Chen trained the model
using 15 fundamental tricks commonly observed in competitive settings,
illustrated in Figure \ref{fig:tricks}. The researcher collected 50 videos per
trick, summing up to a total number of 750 videos. Of these, 45 videos per trick
were allocated for training, and the remaining 5 were reserved for validation.


\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.75 \textwidth]{content/chapters/3_Literature Review/figures/chen-tricks.png}
	\caption{Tricks selected for Chen's models. Reproduced from Chen (2023) \cite{SkateboardAIPaper}}
	\label{fig:tricks}
  \end{figure}
...
In the abstract by Hanciao Chen \cite{SkateboardAIPaper}, extensive experimentation is
