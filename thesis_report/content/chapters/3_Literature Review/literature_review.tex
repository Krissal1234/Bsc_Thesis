...
potential in the realm of sports. The research conducted by K. Host and M. Ivašić-Kos in
...
importance of activity recognition.
\paragraph{} 
...
Techniques such as flipping, rotating, scaling and colour manipulation not only
...
\paragraph{}
After establishing the role of data augmentation in addressing the lack of data available, we move to another important step in computer vision preprocessing: data normalization. This technique, essential for standardising the range and distribution of pixel values in an image, plays a significant role in enhancing model performance, \cite{RobustnessInMLNormalisation}, \cite{RealWorldMicroGraphDataQualityNORMALIZATIONCITE}. In computer vision tasks, where 8-bit image formats like JPEG are common, min-max normalisation is particularly prevalent. This technique adjusts pixel intensity values to a scale between 0 and 1, calculated as follows:\\
\paragraph{}
$\text{Normalized Value} = \frac{\text{Pixel Value} - \text{Min Value}}{\text{Max Value} - \text{Min Value}}$
\paragraph{}
The research by Pei et al. \cite{RobustnessInMLNormalisation}, highlights the importance of this normalisation process, by showing that normalising pixel intensities by dividing them by 255 significantly enhances classification accuracy. This division by 255 is essential because each colour channel in an 8-bit image can represent values from 0 to 255.
On the other hand, the study by de Raad et al. \cite{EffectOnPreProcessingOnCNNForMedicalImageSegmentation} suggests
that the impact of normalization on model performance can vary depending on certain characteristics of the dataset. These contrasting conclusions presented in the studies \cite{RobustnessInMLNormalisation} and \cite{EffectOnPreProcessingOnCNNForMedicalImageSegmentation} suggests that while normalization is an important preprocessing step, its application should be carefully adapted to the characteristics of the dataset.
\paragraph{}

Xudong Jiang \cite{FeatureExtractionForImageRecognitionAndComputerVision}, describes feature extraction as the process of capturing the core attributes of an object by eliminating redundant parts of the data, leaving a set of numerical features that best describe the object and simplify the classification task. This procedure can be applied in a number of ways, however CNN's are a popular choice due their capabilities in detecting complex patterns and enhancing features. CNN's are primarily used in image recognition, serving as an initial step before deploying classification algorithms. This approach is outlined in the study by Manjunath Jogin et al. \cite{FeatureExtractionUsingCNNandDeepLearning}, where they achieved 86\% accuracy rate through the use of CNN's for feature extraction along with several classifiers. Beyond this primary function, CNN's ability to extract complex features from images makes them very effective when coupled with other models for more complex tasks. For instance, when dealing with activity recognition, CNN's can first operate on individual frames to extract spatial features, which can then be passed through a sequence analysis model such as an LSTM, known to be good at understanding temporal dynamics. 
\paragraph{}
Following the discussion on CNN's for feature extraction, it is important to highlight the role of transfer learning in enhancing ML models, especially in fields with limited data like skateboard trick classification. The concept of transfer learning as detailed by the studies \cite{ASurveyOfTransferLearning} and \cite{ApplicationAndAnalysisOfTransferLearningSurvey} involves using a pretrained ML model to leverage its past experience for a new, but related task.
----continue -----
