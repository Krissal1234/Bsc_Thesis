...
available, another important step in computer vision, is data normalization.
This a technique used in preprocessing to standardise the
range and distribution of pixel values in an image, improving model performance,
\cite{RobustnessInMLNormalisation}, \cite{RealWorldMicroGraphDataQualityNORMALIZATIONCITE}.
In computer vision tasks, where 8-bit image formats like JPEG are common,
min-max normalisation is particularly prevalent. The research by Pei et al.
\cite{RobustnessInMLNormalisation}, underscores this, showing that normalizing
pixel intensities by dividing them by 255 to adjust the scale from 0 to 1
significantly enhances classification accuracy. Meanwhile, the study by de Raad
et al. \cite{EffectOnPreProcessingOnCNNForMedicalImageSegmentation} suggests
that the impact of normalization on model performance can vary depending on certain
characteristics of the dataset. These contrasting conclusions presented in the studies
\cite{RobustnessInMLNormalisation} and
\cite{EffectOnPreProcessingOnCNNForMedicalImageSegmentation} suggests that while
normalization is an important preprocessing step, its application should be
carefully adapted to the characteristics of the dataset.
\paragraph{}

Xudong Jiang \cite{FeatureExtractionForImageRecognitionAndComputerVision}, outlines feature extraction as the process of capturing the core attributes of an object. This is achieved by eliminating redundant parts of the data, leaving a set of numerical features that best describe the object and simplify the classification task. 
This procedure can be applied in a number of ways, however CNN's are a popular choice due their capabilities in detecting complex patterns and enhancing features. While CNN's are primarily used in image recognition, as a preliminary step before deploying classification algorithms as outlined in the study by Manjunath Jogin et al. \cite{FeatureExtractionUsingCNNandDeepLearning}, where they achieved 86\% accuracy utilising feature extraction, their functionality extends further. CNN's ability to extract complex features from images makes them very effective when coupled with other models for more complex tasks. For instance, when dealing with activity recognition, CNN's can first operate on individual frames to extract spatial features, which can then be passed through a sequence analysis model such as an LSTM, known to be good at understanding temporal dynamics. This approach is very popular in the field of Activity Recognition. ..---Find paper that shows HAR methods --
Notably the CNN-LSTM architecture is an effective model in accurately recognising human activities, this is outlined in the study by Ronald Mutegeki \cite{CNN-LSTMApproachToHAR}. The author reports this architecture achieving 99\% and 92\% on the iSPL and UCI HAR datasets respectively. These results underscores the strength in  -------
Following the discussion on CNN's for feature extraction, it is important to highlight the concept of transfer learning, a technique 
