...
leisure activity for surfers on flat land to a worldwide
phenomenon~\cite{SkateboardingEncyclobeadia}. This worldwide appeal was further
amplified by its inclusion as an official sport in the 2020 Tokyo Olympic
Games~\cite{SkatebaordingOlimpics}. This event had a significant impact on the
popularity of the sport globally, but particularly evident in the United States, as
demonstrated by the spike in the number of U.S. skateboarding participants
between the years 2020 and 2021, illustrated in Figure~\ref{fig:skate_graph}.
...
\begin{figure}[htbp]
	...
	\caption{Number of skateboarders in the US from 2010 to 2021, measured in
	millions.	Reproduced from~\cite{skatestatistics}, data sourced from Outdoor
	Foundation~\cite{outdoorfounation}.}\label{fig:skate_graph}
\end{figure}

\section{Motivation}
The recent surge in skateboarding's popularity across digital platforms like
YouTube and Instagram has created a potential for innovative technologies to enhance
viewer engagement with skateboarding content. One promising innovation could be
the implementation of on-screen trick identification. This feature would allow
inexperienced viewers to identify tricks performed by skaters in videos through
a digital overlay that labels each manoeuvre.

Traditional methods require manually labelling each trick in a video, a process
that is not only time-consuming but also error-prone. Automating this process
could save time in this regard, but also offer real-time benefits for
live-streamed content, such as skateboard competitions and events. This
automation could provide insightful footage to viewers, further enhancing their
understanding and appreciation of the sport. An Artificial Intelligence (AI)
model capable of accurately classifying skateboard tricks could significantly
...
\item What is the impact of different video preprocessing techniques on classification accuracy?
\end{itemize}
...
\subsection{Aims}
\begin{itemize}
	\item To classify skateboard tricks using frames extracted from videos into their respective classes.
	\item To evaluate and compare the performance of various Deep Learning architectures tailored for skateboard trick classification.
	\item To examine the efficiency of different preprocessing techniques and their influence on the performance of Deep Learning architectures.

\end{itemize}
\subsection{Objectives}\label{1_objectives}
\begin{itemize}
	\item Implement and evaluate a set of Deep learning architectures, using metrics such as accuracy, efficiency and real-time applicability.
	\item Explore and employ suitable frame processing techniques that enhance data quality.
	\item Apply various preprocessing techniques and evaluate them, assessing their impact on model performance and training time.
\end{itemize}
...
specifics detailed in Chapter 5. Chapter 6, discusses the outcomes and evaluates
them against other studies and finally, Chapter 7 presents the possibilities of
future work and the conclusions of this research.
