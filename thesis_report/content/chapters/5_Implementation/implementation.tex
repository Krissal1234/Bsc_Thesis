...
\ref{fig:comparison-sequence-opticalflow-uniformSampling}. Furthermore, experiments showed an increase in
accuracy when using the optical flow method over uniform sampling, further
...
directional information, retaining only the magnitudes. Finally, the code
...
controlling image shifts, brightness, scaling and other transformations. Figure
\ref{fig:originalFrames-againstAugmented} showcases an augmented trick sequence
against its original, split into six frames for illustrative purposes.
...
impacting model performance, thus these were removed or adjusted from the augmentation
parameters.
