...
brightness, scaling and other transformations. Conducted experiments revealed
...
restored the models weights, preventing overfitting. The models all used a
categorical-crossentropy loss function as well as one-hot encoded labels during
training. The output layers made use of a softmax activation function to match
the one-hot encoded lables. 
%mention SGD did better but did take longer


This study compared the results of Stochastic Gradient Descent (SGD) and Adam as
...
This improvement is illustrated in Figure, using the VGG-BiLSTM architecture.
While, Adam initially shows promise with good generalisaiton, it quickly
...
(2016) \cite{SGD_stability_paper}. The results observied with the VGG-BiLSTM
align with the theoretical insights explained by Hardt et al. suggesting that
models trained with SGD are less vulnerable to overfitting.
